...
if (isset($_GET["page"])){
    ...
    if ($_GET["page"] == "" && $_POST) {
        ...
        function grab_image ($url, $setup)
        {
            ...
            $path = "./tmp/" . time() . $filename[0];
            $remotedir = "./tmp/test/";
            $remotepath = $remotedir . time() . $filename[0];
        ...
        $result = getdoc($setup->site . "/index.php?q=" . str_replace(" ", "+", $_POST["q"]), $setup, 0);
        $output = $result[0];
        $success = $result[1];
        $errorcode = $result[2];
        ...
        //print_r($output);
        preg_match_all('@<span style="color:black;">([^"]+)</span>@', $output, $out);
        //print_r($out[1]);
        echo "<br>";
